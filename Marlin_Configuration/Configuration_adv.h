...
// 添加G35命令来读取床角以帮助调整螺丝。需要一个床探针。
//
#define ASSISTED_TRAMMING
#if ENABLED(ASSISTED_TRAMMING)

  // Define from 3 to 9 points to probe. 定义3到9个点进行探针。
  // #define TRAMMING_POINT_XY { {  20, 20 }, { 180,  20 }, { 180, 180 }, { 20, 180 } }
  #define TRAMMING_POINT_XY { {  30, 30 }, { 200,  30 }, { 200, 200 }, { 30, 200 } }

  // Define position names for probe points. 为探针点定义位置名称。
  #define TRAMMING_POINT_NAME_1 "Front-Left"
  #define TRAMMING_POINT_NAME_2 "Front-Right"
  #define TRAMMING_POINT_NAME_3 "Back-Right"
  #define TRAMMING_POINT_NAME_4 "Back-Left"

  #define RESTORE_LEVELING_AFTER_G35    // Enable to restore leveling setup after operation 启用此选项以在操作后恢复水平设置
  //#define REPORT_TRAMMING_MM          // Report Z deviation (mm) for each point relative to the first 报告每个点相对于第一个点的Z偏差（mm）

  #define ASSISTED_TRAMMING_WIZARD    // Add a Tramming Wizard to the LCD menu 在LCD菜单中添加一个校平向导

  // #define ASSISTED_TRAMMING_WAIT_POSITION { X_CENTER, Y_CENTER, 30 } // Move the nozzle out of the way for adjustment 将喷嘴移开以进行调整
...
//#define LCD_BACKLIGHT_TIMEOUT_MINS 1  // (minutes) Timeout before turning off the backlight
  ...
  // 编译报错 "SLIM_LCD_MENUS disables \"Advanced Settings > Probe Offsets > PROBE_OFFSET_WIZARD.\"" 禁用 SLIM_LCD_MENUS 需要15000字节左右空间
 ...
 * 高级暂停以更换耗材
 *  - 添加G代码M600进行耗材更换。
 *  - 此功能是默认FILAMENT_RUNOUT_SCRIPT所需的。
 *
 * 要求：
 *  - 为了启用耗材更换停车功能，请启用并配置NOZZLE_PARK_FEATURE。
 *  - 为了用户交互，请启用LCD显示器、HOST_PROMPT_SUPPORT或EMERGENCY_PARSER。
 *
 * 启用PARK_HEAD_ON_PAUSE以添加G代码M125暂停并停车。
 */
...
// Custom Menu: Main Menu
  ...
  #define MAIN_MENU_ITEM_1_DESC "disable software endstops"
  #define MAIN_MENU_ITEM_1_GCODE "M211 S0"
